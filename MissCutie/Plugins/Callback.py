...
from MissCutie import BOT_USERNAME, MUSIC_BOT_NAME, app, db_mem
...
from MissCutie.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from MissCutie.Decorators.admins import AdminRightsCheckCB
from MissCutie.Decorators.checker import checkerCB
from MissCutie.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup,
                          secondary_markup2)
from MissCutie.Utilities.changers import time_to_seconds
from MissCutie.Utilities.chat import specialfont_to_normal
from MissCutie.Utilities.paste import isPreviewUp, paste_queue
from MissCutie.Utilities.theme import check_theme
from MissCutie.Utilities.thumbnails import gen_thumb
from MissCutie.Utilities.timer import start_timer
...
async def check_playlist(_, CallbackQuery):
    ...
    for saifalisew1508 in _playlist:
        j += 1
        _note = await get_playlist(user_id, saifalisew1508, genre)
        title = _note["title"]
        duration = _note["duration"]
        msg += f"{j}- {title[:60]}\n"
...
async def del_playlist(_, CallbackQuery):
    ...
    await CallbackQuery.answer()
    for saifalisew1508 in _playlist:
        await delete_playlist(user_id, saifalisew1508, genre)
    await CallbackQuery.message.reply_text(
